...
export function RunPanel({ clientId, onRunFinished }) {
  ...
  function pollRun(id) {
    ...
    pollRef.current = setInterval(async () => {
      const data = await api.get(`/api/clients/${clientId}/tds/runs/${id}`);
      setRun(data);
  ...
  async function startRun(e) {
    ...
    setStarting(true);
    try {
      const { runId: newRunId } = await api.post(`/api/clients/${clientId}/tds/runs`, { period });
      setRunId(newRunId);
  ...
  async function submitOtp(e) {
    ...
    setError(null);
    try {
      await api.post(`/api/clients/${clientId}/tds/runs/${runId}/otp`, { otp });
      setOtp("");
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div className="card">
      <h2 style={{ marginTop: 0 }}>Fetch TDS data</h2>
      <form onSubmit={startRun} className="row">
        ...
        <button type="submit" disabled={starting}>
          {starting ? "Starting..." : "Fetch TDS data"}
        </button>
      </form>
